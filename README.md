...
| Docs | SpringDoc OpenAPI 2.8 (Swagger) |

## 주요 기능

- 등록된 RSS 출처를 매시간 자동 수집 (Quartz 스케줄러)
- 썸네일 추출 (Enclosure → Media RSS → content:encoded → OG Image 순 fallback)
- 태그 자동 분류 (RSS 카테고리 또는 키워드 분석)
- 신규 콘텐츠를 Kafka로 AI 요약 요청 비동기 전송
...
| POST | `/api/v1/collector/collect` | 수동 수집 트리거 (`?full=true` 전체 수집) |
| POST | `/api/v1/collector/collect/{sourceId}` | 단건 수집 |
| GET | `/api/v1/collector/logs` | 수집 로그 목록 |
| GET | `/api/v1/collector/logs/failures` | 실패 로그 목록 |

## Kafka 토픽

| 토픽 | 방향 | 설명 |
|------|------|------|
| `content-summary-request` | 발행 | 신규 콘텐츠 AI 요약 요청 |
